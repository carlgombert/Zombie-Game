...
public class Zombie extends GameObject{
	...
	public Zombie(ID id, Handler handler) {
		super(0, 0, id);
		startingPosition();
		
		this.handler = handler;
		
		zombie = Game.addImage(100, 100, "res/zombie" + (int)Math.floor(Math.random() * (3 - 1 + 1) + 1) + ".png");
	}
		...
		for(int i = 0; i < handler.object.size(); i++) {
			...
			if(tempObject.getId() == ID.Bullet) {
				if(getBounds().intersects(tempObject.getBounds())){
					...
					int temp = (int) Math.floor(Math.random() * (10 - 1 + 1) + 1);
					if(temp == 5) {
						handler.addObject(new Medpack(x, y, ID.Medpack, handler));
					}
				}
			}
